...
def plot_line_graph(x_values, y_values):
    ...
    plt.xlabel('시간')
    plt.ylabel('상승개체수-하강개체수')
    plt.title('군집의 상하 운동 경향성')
    plt.grid(True)
    plt.show()
...
def extract_motion_vectors(video_path, output_path):
    ...
    while True:
        ...
        diff = positive_y_count - negative_y_count
        sumdiff += diff

        x_values.append(framenum)
        y_values1.append(sumdiff)
        ...
        cv2.putText(frame_with_motion_vectors, f'cnt DIF: {diff}', (y_count_pos[0], y_count_pos[1] - 50), font, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.putText(frame_with_motion_vectors, f'SUM DIF: {sumdiff}', (y_count_pos[0], y_count_pos[1] - 80), font, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
...
# Example usage
video_path = 'test.mp4'
output_path = 'res_dif_1.avi'
extract_motion_vectors(video_path, output_path)

# Example usage
plot_line_graph(x_values, y_values1)
